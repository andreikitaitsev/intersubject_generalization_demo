...
inp_base=Path('/scratch/akitaitsev/intersubject_generalizeation/linear/generic_decoding/increment_train_data/')
    ...
    for prepr in args.preprs:
        for n_comp in args.n_comps:
            for num, step in enumerate(args.steps):
                filepaths.append(inp_base.joinpath(meth, prepr, n_comp, '50hz','time_window13-40', ('step_'+\
                    str(step))))
                percents.append( str((num+1)*10))
...
if not args.save_fig:
    plt.show()
if args.save_fig:
    out_path=Path('/scratch/akitaitsev/intersubject_generalizeation/linear/generic_decoding/plots/increment_train_data/50hz/')
