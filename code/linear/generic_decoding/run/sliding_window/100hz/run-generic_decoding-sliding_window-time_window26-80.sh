...
do
    ...
    do
        ...
        do
            out_dir[$ind]=$out_base$prepr"/"$n_comp"/100hz/time_window26-80/"
            real_files[$ind]=$real_base$prepr"/"$n_comp"/100hz/time_window26-80/shared_test.pkl"
            pred_files[$ind]=$pred_base$prepr"/"$n_comp"/100hz/time_window26-80/Y_test_predicted_"$dtype".pkl"
            dtypes[$ind]=$dtype
            ((ind=ind+1))
