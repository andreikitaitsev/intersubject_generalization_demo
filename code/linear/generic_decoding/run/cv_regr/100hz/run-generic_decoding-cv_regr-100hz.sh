...
do 
    ...
    do
        real_files[$ind]=$real_base"/"$method"/main/"$prepr"/"$n_comp"/100hz/time_window26-80/shared_test.pkl"        
        pred_files[$ind]=$pred_base"/"$method"/"$prepr"/"$n_comp"/100hz/time_window26-80/Y_test_predicted_.pkl"
        out_dirs[$ind]=$out_base"/"$method"/"$prepr"/"$n_comp"/100hz/time_window26-80/"
        ((ind=ind+1))
    done
done
